...
- **CQRS at the controller boundary, service classes underneath** in `Identity` — controllers bind a `Contracts` DTO and dispatch an `internal` mediator command/query for every write and for user search, but the handlers forward to `UserService`/`RoleService` (except `SearchUserQueryHandler`, which hits `UserManager` directly). A half-migrated state, not a settled dual pattern. See `modules/Identity.md`.
