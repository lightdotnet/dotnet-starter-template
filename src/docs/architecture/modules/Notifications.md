...
Owns storage and real-time delivery of system/user notifications: persisting notification records (sender, recipient, title, message, optional deep-link URL, read/archived status) and pushing them live to connected clients over SignalR. Exposes two surfaces: an admin-facing "browse + send" capability gated by explicit permissions, and a self-service "my notifications" surface auto-scoped to the calling user via `ICurrentUser` (no extra permission required — any authenticated user can read/mark-read their own notifications). Also carries a distinct "force logout" push message that reuses the same SignalR channel as a live session-invalidation signal, without creating a stored notification record. The module also exposes two in-process capabilities through its `Contracts` seam for other business modules to call via DI: outbound email-sending (`IMailService`, consumed by `Identity.Api`) and the combined persist-and-push notification (`INotificationService.SendAsync`, consumed by `Approval.Api`) — see Dependencies/Depended On By.
...
Every action above dispatches through a mediator command/query under `Application/Notifications/` (`SendNotificationCommand`, `ForceLogoutCommand`, `MarkNotificationReadCommand`, `SearchNotificationsQuery`, `GetNotificationQuery`, `CountUnreadNotificationsQuery`) rather than the controller calling `INotificationService`/`IHubService` directly — both controllers only bind the `Contracts` DTO/route param and call `Mediator.Send`. `SendNotificationCommandHandler` is a thin pass-through to `INotificationService.SendAsync` (persist + SignalR push composed inside the service, see Notable Conventions — this composition used to live in the handler itself before `SendAsync` was promoted onto the `Contracts` seam for cross-module use); `ForceLogoutCommandHandler` instead calls `IHubService` directly, since a force-logout push has no stored record to persist. This mirrors `Identity`'s CQRS-entrypoint convention (see `Identity.md` § Notable Conventions) rather than resolving the open question there — see `../../known-debt.md`, D1.
...
**In-app notification push (not HTTP-exposed)**: `INotificationService.SendAsync(fromUserId, fromName, toUserId, SystemMessage, ct)` (`Notifications.Contracts/Services/INotificationService.cs`) is the cross-module equivalent of the admin `POST notification` endpoint — it composes `SaveAsync` (persist a `Notification` row) then `IHubService.SendAsync` (push over SignalR) in one call, for another module to invoke in-process via DI instead of going through HTTP/mediator. `NotificationService` (`Notifications.Api/Services/NotificationService.cs`) now takes `IHubService` via constructor injection to implement it. Named the same as `IMailService.SendAsync` but otherwise unrelated — don't conflate the two when reading call sites. Consumers: `SendNotificationCommandHandler` (in-module, backing the admin endpoint) and `Approval.Api`'s `ApprovalStepPendingEventHandler`/`ApprovalFinalizedEventHandler` (cross-module, see `Approval.md`).
...
`Identity.Api` references `Notifications.Contracts` — its `UserCreatedEventHandler` takes a DI-injected `IMailService` and calls `SendFromSystemAsync(...)` to send a welcome email on user creation (wrapped in try/catch; a failure only logs, it doesn't fail the request). This is the only cross-module reference between `Identity` and `Notifications` in either direction, and it's compliant with the module-boundary rule — it reaches only `Notifications.Contracts` (the seam), never `Notifications.Api`'s internals.

`Approval.Api` also references `Notifications.Contracts` — its `ApprovalStepPendingEventHandler`/`ApprovalFinalizedEventHandler` take a DI-injected `INotificationService` and call `SendAsync(...)` to notify the relevant approver/requester when a step becomes pending or a request is finalized (see `Approval.md`). Same compliance shape as `Identity.Api`'s dependency — only `Notifications.Contracts` is reached, never `Notifications.Api`'s internals.

`Notifications.Api` itself is otherwise referenced only by `StarterKit.WebApi` (composition-root host) and `src/Migrations/MSSQL/MSSQL.csproj` (migrations tooling — `PostgreSQL.csproj`/`Sqlite.csproj` do not reference `Notifications.Api`, and neither has a `Notifications` migration; only MSSQL has one) — verified via `ProjectReference` search across all `.csproj` files under `src/`. Client-side: `clients/admin/src/features/notifications/` is the only verified consumer of the module's HTTP surface, calling `notification`/`user_notification` over HTTP and connecting directly to `/signalr-hub` (proxied same-origin via the admin app's `next.config.ts` `rewrites()`); the admin client has no path to `IMailService`/`INotificationService.SendAsync` — both are in-process C# capabilities, not HTTP endpoints.
