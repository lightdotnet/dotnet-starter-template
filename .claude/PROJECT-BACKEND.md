...
- **Pagination**: `QueryableResultExtensions.ToPagedAsync` (`src/Persistence/Extensions/`) now clamps `pageSize` to a max of 100 (`Math.Min(pageSize, 100)`, previously unbounded) — a shared-kernel-level change affecting any current/future paginated endpoint, including `Identity`'s new `UserService.SearchAsync`.
