...
2. **Confirm/record key decisions before generating files**, if not already settled in `.claude/PROJECT-BACKEND.md`/`.claude/ARCHITECTURE-BACKEND.md` (backend) or `.claude/PROJECT-CLIENTS.md`/`.claude/ARCHITECTURE-CLIENTS.md` (clients):
...
9. **Update docs only if requested**: `.claude/PROJECT-BACKEND.md` (Backend Modules table) and/or `.claude/PROJECT-CLIENTS.md` (Client Apps table) can be updated to reflect the new skeleton, but only if the user asks — otherwise report the created structure conversationally.
...
- A summary of what was created and any decisions made that should be recorded (offer to update `.claude/PROJECT-BACKEND.md`/`.claude/PROJECT-CLIENTS.md`).
