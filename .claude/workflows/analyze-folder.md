...
4. **Generate or update documentation only if requested**:
   - If asked, write/update a scoped doc under `.claude/docs/generated/backend/<path>/` or `.claude/docs/generated/clients/<app-name>/<path>/` using the appropriate template from `.claude/docs/templates/` (project-overview, module-overview, client-app-overview, or a custom scope-appropriate template).
