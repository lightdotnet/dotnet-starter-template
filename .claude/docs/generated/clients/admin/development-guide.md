...
---
_Generated: 2026-08-04 (resynced — updated the "User management UI"/"Role management UI" bullets for the edit dialogs' new on-open picklist self-fetch; updated the "Notification management UI" bullet for `notifications-page.tsx` dropping `getAllUsers` and `user-select.tsx`'s on-demand-search rewrite; updated the "Select / combobox input" bullet — `Combobox` now has one consumer) — scope: client app "admin" — see .claude/CLAUDE.md for update rules._
