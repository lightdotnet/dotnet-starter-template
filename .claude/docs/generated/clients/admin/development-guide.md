...
| Add a new backend endpoint call | Create one file under the relevant `features/<name>/api/`, wrapping `requestJson`/`requestVoid` from `lib/server/backend-api.ts` (pre-wired with bearer-token auth and `client: ApiClients.Backend` — do **not** import from `lib/server/http.ts` directly unless you're one of the 3 pre-session-cookie exceptions, see "Server-only API plumbing" below) via `guardCall`/`guardResponseCall`/`guardRawCall` (`lib/server/call-guard.ts`); export it from the feature's `index.ts` barrel. |
...
- **User management UI (`/identity/users`)**: `features/users/components/users-page.tsx` (Server Component: session/permission gate via `lib/server/authorization.ts` + `features/users/constants/permissions.ts`, fetches via `search-users.ts`), `features/users/components/users-data-table.tsx` (client wrapper around the shared `DataTable`, owns the create/edit/delete dialogs' open state), `features/users/components/create-user-dialog.tsx` (create form), `edit-user-dialog.tsx` (edit + password reset, fetches full detail on open via `get-user-detail-action.ts` — and, new this sync, also self-fetches the role catalog via `get-all-roles-action.ts` on open, since `users-page.tsx` no longer preloads and passes it down), `delete-user-dialog.tsx` (confirm + delete).
- **Role management UI (`/identity/roles`)**: `features/roles/components/roles-page.tsx` (Server Component: permission gate via `features/roles/constants/permissions.ts`, fetches all roles — **no longer** the permission catalog this sync), `roles-data-table.tsx` (client wrapper, client-side search filter — no backend search endpoint), `create-role-dialog.tsx` (name/description only — the backend accepts no claims on create), `edit-role-dialog.tsx` (name/description + a permissions checklist, fetches full role detail on open via `get-role-detail-action.ts` — and, new this sync, also self-fetches the permission catalog via `get-permissions-action.ts` on open, rather than it being sourced from the page), `delete-role-dialog.tsx`.
