...
- **Reusable list/table building block**: `components/shared/data-table/` (`DataTable`, toolbar, pagination, `types.ts` — including optional per-column client-side sort via `sortable`/`sortValue`) — generic, no feature knowledge; used by `features/users`, `features/roles`, and `features/notifications`.
...
---
_Generated: 2026-08-04 (resynced — added a "Select / combobox input" where-to-look bullet for the new `components/ui/combobox.tsx`; noted `popover.tsx`/`command.tsx` were hand-written rather than CLI-generated) — scope: client app "admin" — see .claude/CLAUDE.md for update rules._
