...
| Add a new list/table page | Reuse `components/shared/data-table` (`DataTable<TData>`) — define `DataTableColumn<TData>[]`, pass `data`/`isLoading`/`error`/pagination props/callbacks. Two reference patterns exist depending on whether the backend endpoint supports search/pagination: `features/users/components/{users-page,users-data-table}.tsx` (server-driven — Server Component fetch + gate on permission, Client Component wrapper drives URL-param search/pagination via `useTransition`) if it does; `features/roles/components/{roles-page,roles-data-table}.tsx` (client-side filtering over a full list, static single-page pagination props) if it doesn't. |
...
- **Reusable list/table building block**: `components/shared/data-table/` (`DataTable`, toolbar, pagination, `types.ts` — including optional per-column client-side sort via `sortable`/`sortValue`) — generic, no feature knowledge; used by both `features/users` and `features/roles`.
...
- **Nav structure/labels**: each nav-bearing feature owns its own entry — `features/{dashboard,users,roles}/constants/nav-item.ts` (`DASHBOARD_NAV_ITEM`/`USERS_NAV_ITEM`/`ROLES_NAV_ITEM`); `src/constants/nav-items.ts` (typed via `types/nav.ts`) assembles those into `NAV_ITEMS`, only declaring the "Identity" group and "Settings" leaf itself.
