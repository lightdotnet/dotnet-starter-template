...
src/
  ...
  features/
    ...
    home/                   (new this sync, replaces the deleted dashboard/ — see below) components/{home-page,
    ...
    users/                  api/*.ts (15 endpoint/action files — the original 8 minus this sync's deletion of the
                             unused get-user-by-username.ts (7 remain, zero remaining references anywhere in src/),
                             plus create-user-action, update-user-action, force-password-action, delete-user-action,
                             get-user-detail-action, search-users-action (all "use server"), and — new this sync —
                             get-domain-user.ts (plain endpoint wrapper) + get-domain-user-action.ts ("use server")),
                             ...
                             and — new this sync — `AUTH_PROVIDER_SELECT_OPTIONS`, shared by create-user-dialog.tsx
                             and edit-user-dialog.tsx),
    ...
    notifications/          api/*.ts (8 endpoint files: get-notifications, get-my-notifications(+action),
                             get-unread-count(+action), mark-notification-read(+action), send-notification(+action),
                             get-signalr-token-action), hooks/use-notifications.ts, context/notifications-provider.tsx
                             (new this sync — `NotificationsProvider`/`useNotificationsContext`),
                             ...
                             notification-detail}.tsx (last 3 new this sync — the two-pane inbox rendered on the
                             new Home page), constants/{permissions,nav-item}.ts, types/notification.ts, index.ts
  components/
  ...
  lib/
    server/                 config.ts, http.ts, call-guard.ts, session-cookie.ts, session.ts, authorization.ts,
                             ...
                             get-signalr-token-action.ts, see Key Design Patterns; all 15 files listed here now
                             start with `import "server-only";` — new this sync, a compile-time guard, see Key
                             Design Patterns)
...
features/auth/api/refresh-session-action.ts -> "use server"; lib/server/session (getSession), lib/server/refresh-session
                                        ...
                                        is not otherwise documented here — out of scope for this pass). Was the
                                        lone `next/cache` user in this app until this sync — the 7 users/roles/
                                        notifications mutation actions below (create/update/delete-user-action,
                                        create/update/delete-role-action, send-notification-action) now follow
                                        the same self-invalidating `revalidatePath` pattern, see Key Design
                                        Patterns
...
features/home/index.ts              -> ./components/home-page, ./constants/nav-item (HOME_NAV_ITEM) — new this sync,
                                        replaces the deleted features/dashboard/index.ts
...
features/home/constants/nav-item.ts -> lucide-react (Home), types/nav — new; label "Home", href "/", no permission

features/users/index.ts             -> ./components/users-page, ./api/{search-users,get-all-users,get-user-by-id,
                                        create-user,update-user,delete-user,force-password},
                                        ./constants/permissions (USERS_PERMISSIONS), ./constants/nav-item (USERS_NAV_ITEM),
                                        ./types/user — **changed this sync**: no longer re-exports `get-user-by-username`
                                        (`api/get-user-by-username.ts` was deleted — unused, zero remaining references
                                        anywhere in src/)
...
features/users/components/create-user-dialog.tsx -> components/ui/{alert,button,combobox,dialog,input,label,spinner},
                                        ...
                                        features/users/constants/auth-provider — **changed this sync**: gained a
...
features/users/components/edit-user-dialog.tsx -> components/ui/{alert,button,checkbox,combobox,dialog,input,label,
                                        ...
                                        and the password-reset form). **Changed this sync**: no longer defines its
                                        own `AUTH_PROVIDER_OPTIONS`/`AUTH_PROVIDER_SELECT_OPTIONS` locally — now
                                        imports `AUTH_PROVIDER_SELECT_OPTIONS` from the new
                                        features/users/constants/auth-provider.ts (shared with create-user-dialog.tsx).
                                        Its `authProvider` default state and `isLocalAccount` check changed from
                                        `"Local"`/`.toLowerCase() === "local"` to `""`/`authProvider === ""` to match
...
features/users/api/create-user-action.ts -> "use server"; features/user-profile (resolveSession), features/users/api/create-user,
                                        features/users/types/user, next/cache (revalidatePath — new this sync, "/identity/users")
                                        — **changed this sync**: password is now only required when
...
features/users/api/get-domain-user-action.ts (new this sync) -> "use server"; features/user-profile (resolveSession),
...
features/users/api/{update-user-action,force-password-action,delete-user-action,get-user-detail-action}.ts
                                     -> "use server"; features/user-profile (resolveSession),
                                        ...
                                        (revalidatePath — new this sync, "/identity/users"); force-password-action
                                        and get-user-detail-action do not — neither one changes list-page data)
features/users/api/*.ts (8 remaining files) -> lib/server/backend-api, lib/server/call-guard, types/api, features/users/types/user
                                        (changed this sync — was lib/server/http; these ordinary endpoint files
                                        no longer take an accessToken parameter, see Key Design Patterns). **New
                                        this sync**: get-domain-user.ts (returns `Result<DomainUserDto>`, calling
                                        `GET user/get_domain_user/{userName}`) joins this group
...
features/roles/api/{create-role-action,update-role-action,delete-role-action,get-role-detail-action}.ts
                                     -> "use server"; features/user-profile (resolveSession),
                                        ...
                                        next/cache (revalidatePath — new this sync, "/identity/roles");
...
features/notifications/index.ts     -> ./components/{notification-bell,notifications-page,notification-inbox}
                                        (notification-inbox added this sync), ./hooks/use-notifications,
...
features/notifications/hooks/use-notifications.ts -> @microsoft/signalr, features/notifications/api/{get-my-notifications-action,
                                        ...
                                        features/notifications/types/notification — changed this sync: markAsRead(id)
                                        now returns Promise<boolean> (was Promise<void>); refresh(status?) gained an
...
features/notifications/context/notifications-provider.tsx -> react (createContext/useContext),
                                        features/notifications/hooks/use-notifications — new this sync; calls
...
features/notifications/components/notification-bell.tsx -> components/ui/{badge,button,dropdown-menu,empty,tabs},
                                        ...
                                        (useNotificationsContext — changed this sync, was hooks/use-notifications
                                        directly), features/notifications/types/notification (imported by
                                        components/layout/topbar.tsx via direct file path, not this feature's own
                                        barrel — see the exceptions note below). Changed this sync: tab filter
                                        (All/Unread/Archived) now calls refresh(status) from context instead of
...
features/notifications/components/notification-inbox.tsx -> components/ui/{badge,empty,pagination,tabs},
                                        ...
                                        — new this sync; "use client"; two-pane grid layout (list + detail), local
...
features/notifications/components/notification-list.tsx -> lib/shared/utils, features/notifications/types/notification
                                        — new this sync; the inbox's master-list pane (unread-dot indicator, title,
...
features/notifications/components/notification-detail.tsx -> next/link, components/ui/{badge,empty}, lucide-react,
                                        features/notifications/types/notification — new this sync; the inbox's
...
features/notifications/api/get-my-notifications-action.ts -> "use server"; features/user-profile (resolveSession),
                                        features/notifications/api/get-my-notifications — changed this sync: signature
                                        went from a hardcoded no-arg call to
                                        `(params: Omit<NotificationLookupParams, "toUserId"> = {})`, forwarded
                                        straight through to the already-parameterized getMyNotifications(); fully
                                        backward-compatible with existing no-arg callers (use-notifications.ts's
                                        refresh()). New callers this sync: features/home/components/home-page.tsx
...
features/notifications/api/mark-notification-read-action.ts -> "use server"; features/user-profile (resolveSession),
                                        ...
                                        deliberately did NOT gain revalidatePath this sync — its only caller is the
...
features/notifications/api/send-notification-action.ts -> "use server"; features/user-profile (resolveSession),
                                        ...
                                        next/cache (revalidatePath — new this sync, "/notifications")
...
| Nav item references a route with no `page.tsx` (`/settings`, `/administration`) | Low | `/identity/users` and `/identity/roles` both have real pages. `/settings` still 404s if followed; **found via this sync's verification**: the assembled nav tree's group node — relabeled "Administration"/`/administration` this session (previously "Identity"/`/identity`) — also has no `page.tsx` of its own, same shape as `/settings`. |
...
| ~~Dashboard still renders hardcoded mock data (`features/dashboard/api/sample-data.ts`)~~ — **resolved this sync** | — | `features/dashboard/` was deleted outright and replaced by `features/home/`, a real Server Component calling `resolveSession()` and `getMyNotificationsAction()`. There is no longer any page in this app that skips the backend. |
| No `not-found.tsx` anywhere under `app/` | Low (cosmetic) | **New this sync**: navigating to an unmatched route renders Next.js's default 404 within the root layout only, unmounting the entire `(dashboard)/layout.tsx` subtree (`AppShell`, `TopBar`, `NotificationBell`, and — if the current page is `/` — `NotificationInbox`), which intentionally stops any open SignalR connection mid-flight. This surfaces as an abnormal WebSocket closure (code 1006), which SignalR's client used to log via `console.error` by default; `use-notifications.ts` now adds `.configureLogging(LogLevel.Critical)` to silence that specific noise (see Key Design Patterns), but the underlying missing-`not-found.tsx` fact itself is unchanged — worth adding a real one if a friendlier 404 experience is ever needed, independent of this logging fix. |
...
| `components/ui/button.tsx` hand-modified beyond shadcn CLI output (`loading` prop, `cursor-pointer`) | Low | Unchanged — re-running the shadcn CLI would silently drop these customizations unless done carefully. **New this sync**: `tabs.tsx` (`cursor-pointer` on `TabsTrigger`) and `dialog.tsx` (`max-h`/`overflow-y-auto` on `DialogContent`, plus the pre-existing `relative`/portal-container additions) now carry the same kind of hand-modification risk. |
