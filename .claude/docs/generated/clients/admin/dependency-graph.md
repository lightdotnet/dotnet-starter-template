...
| `features/users/components/edit-user-dialog.tsx` | `components/ui/{alert,button,checkbox,dialog,input,label,spinner,tabs}.tsx`, `components/select` (barrel — `EntitySelect`, status/authProvider), `components/toast` (barrel — `notifySuccess`), `features/users/api/{get-user-detail-action,update-user-action,force-password-action}.ts`, `features/roles/types/role.ts` (`RoleDto`), `types/user.ts` | Fetches full user detail on open (see [architecture.md](./architecture.md#key-design-patterns)). Status/authProvider switched from `components/ui/select.tsx` (Radix, removed this sync) to `EntitySelect` |
...
| `features/notifications/components/user-select.tsx` | `components/select` (barrel — `SearchSelect`), `lib/shared/user-display.ts`, `types/user.ts` | Searchable user picker, feature-owned (not promoted to `components/shared/`); consumed by both `notifications-data-table.tsx` and `send-notification-dialog.tsx`. Rewritten this sync on `SearchSelect` (was a search `<Input>` stuffed inside a Radix `SelectContent`); dropped its `allOption` prop in favor of `SearchSelect`'s own `placeholder`, added `onClear` |
...
| `components/foundation/use-floating-popover.ts` | `@floating-ui/react` (`useFloating`, `useClick`, `useDismiss`, `useRole`, `flip`/`shift`/`offset`/`size`, `autoUpdate`) | New — `enableClick`/`role`/`maxPanelHeight` options; returns raw interaction hooks (not pre-merged via `useInteractions`) so callers combine them with list-navigation/typeahead in one merge point |
| `components/foundation/use-listbox.ts` | `@floating-ui/react` (`useListNavigation`, `useTypeahead`) | New — `enableTypeahead` option (default `true`); text-input-triggered components pass `false` (see architecture.md Key Design Patterns for the bug this avoids) |
| `components/foundation/use-async-options.ts` | `components/foundation/types.ts` | New — debounced fetch + `AbortController` cancellation; `shouldFetch` gating avoids a `setState`-in-effect anti-pattern |
| `components/foundation/use-virtual-list.ts` | `@tanstack/react-virtual` (`useVirtualizer`) | New |
| `components/foundation/options-list.tsx` | `lucide-react`, `lib/shared/utils.ts`, `components/foundation/{use-virtual-list,types}.ts` | New — shared listbox row renderer, virtualizes past 50 options, sets `aria-setsize`/`aria-posinset` |
| `components/foundation/floating-overlay.tsx` | `@floating-ui/react` (`FloatingPortal`, `FloatingOverlay`, `FloatingFocusManager`), `lib/shared/utils.ts` | New — `lockScroll={false}` kept explicit; used only by `command/command-palette.tsx` |
| `components/select/entity-select.tsx` | `@floating-ui/react` (`FloatingPortal`, `useInteractions`), `lucide-react`, `lib/shared/utils.ts`, `components/foundation/{use-floating-popover,use-listbox,options-list,types}.ts` | New — button-triggered, `role="combobox"` (WAI-ARIA "select-only" pattern) |
| `components/select/search-select.tsx` | Same `components/foundation/*` set as `entity-select.tsx`, `lucide-react` | New — text-input-triggered; `enableClick: false`, `enableTypeahead: false` |
| `components/select/async-select.tsx` | `components/foundation/use-async-options.ts` plus the same set as `search-select.tsx` | New — remote-search combobox; `minChars` defaults to 3; no current UI consumer |
| `components/select/multi-select.tsx` | Same `components/foundation/*` set as `search-select.tsx` | New — chip-based multi-select |
| `components/select/index.ts` | `./entity-select.ts`, `./search-select.ts`, `./async-select.ts`, `./multi-select.ts` | New — barrel |
| `components/command/command-palette.tsx` | `@floating-ui/react` (`useFloating`, `useDismiss`, `useRole`, `useInteractions`), `lucide-react`, `lib/shared/utils.ts`, `components/foundation/{floating-overlay,use-listbox,use-virtual-list}.ts` | New — Cmd/Ctrl+K overlay; no UI consumer yet |
| `components/command/command-palette-provider.tsx` | `components/command/command-palette.tsx` | New — global Cmd/Ctrl+K keydown listener via React Context |
| `components/command/index.ts` | `./command-palette.tsx`, `./command-palette-provider.tsx` | New — barrel |
...
| `@floating-ui/react` | `^0.27.20` | New — positioning/interaction primitives underlying `components/foundation/*`, `components/select/*`, `components/command/*` |
| `@tanstack/react-virtual` | `^3.14.9` | New — headless list virtualization underlying `components/foundation/use-virtual-list.ts` |
...
---
_Generated: 2026-08-03 (resynced — added `components/foundation/select/command` import rows and `@floating-ui/react`/`@tanstack/react-virtual` packages; removed `components/ui/select.tsx` references; updated DataTable/edit-user-dialog/user-select rows) — scope: client app "admin" — see .claude/CLAUDE.md for update rules._
