...
| `features/users/components/edit-user-dialog.tsx` | `components/ui/{alert,button,checkbox,combobox,dialog,input,label,spinner,tabs}.tsx`, `components/toast` (barrel — `notifySuccess`), `features/users/api/{get-user-detail-action,update-user-action,force-password-action}.ts`, `features/roles/types/role.ts` (`RoleDto`), `types/user.ts` | Fetches full user detail on open (see [architecture.md](./architecture.md#key-design-patterns)). Status/authProvider now use `components/ui/combobox.tsx` (`Combobox`) — `components/select` (`EntitySelect`), used here previously, is gone this sync |
...
| `features/notifications/components/notifications-data-table.tsx` | `components/shared/data-table` (barrel), `components/ui/native-select.tsx`, `features/notifications/components/{send-notification-dialog,user-select}.tsx`, `lib/shared/user-display.ts`, `features/notifications/types/notification.ts`, `types/user.ts` | Status filter switched from `components/ui/select.tsx` (Radix, removed in a prior sync) to `native-select.tsx`, with a real reselectable placeholder replacing a fake "all" option. **New this sync**: status + recipient filters render through `DataTable`'s new `customSearch` slot (moved out of a standalone block above `<DataTable>`) and no longer auto-navigate per change — local "pending" state is applied to the URL only when the customSearch "Search" button (`onCustomSearch`) is clicked |
...
| `features/notifications/components/user-select.tsx` | `components/ui/combobox.tsx` (`Combobox`), `lib/shared/user-display.ts`, `types/user.ts` | Searchable user picker, feature-owned (not promoted to `components/shared/`); consumed by both `notifications-data-table.tsx` and `send-notification-dialog.tsx`. Migrated this sync from `components/select`'s `SearchSelect` (now gone) to `Combobox` — `UserSelect`'s own props unchanged, so neither consumer needed a change |
...
None found among internal module imports. `components/ui/*` is **no longer an absolutely strict leaf layer as of this sync** — `components/ui/dialog.tsx` and `components/ui/popover.tsx` both import `components/foundation/portal-container.ts`, a deliberate, narrow exception (a dependency-free React Context) added to fix a Dialog/Popover nested-scroll bug (see architecture.md Key Design Patterns). It remains true that `components/ui/*` never imports from `components/layout/*`, `components/theme/*`, or `features/*`, and no cycle results — `components/foundation/*` doesn't import back from `components/ui/*`. Cross-feature imports observed so far (`features/auth` → `features/user-profile`, `features/users` → `features/user-profile`, `features/users` → `features/roles`, `features/notifications` → `features/user-profile`, `features/notifications` → `features/users`) go one direction only, through the target feature's barrel (with three narrow, reasoned exceptions — `constants/nav-items.ts`, `components/layout/user-menu.tsx`, and `components/layout/topbar.tsx` — that import a single plain-data/single-component file directly instead; see architecture.md). `components/shared/data-table/*`, `components/shared/object-viewer/*`, and `components/toast/*` are consumed by feature code but import nothing from `features/*` themselves, so no cycle there either. `components/foundation/*` and `components/command/*` import only `@floating-ui/react`/`@tanstack/react-virtual`/`lucide-react`/`react`/each other — no `features/*` dependency, no cycle. (`components/select/*`, the third member of this group in the previous sync, is deleted.) One dependency-direction reversal is worth flagging (not a cycle, but atypical): `lib/server/refresh-session.ts` imports `features/auth/api/refresh-token.ts` directly — `lib/server/*` normally sits *below* `features/*`, but here it reaches up into one feature's `api/` file. No cycle results since `features/auth` doesn't import back from `lib/server/refresh-session.ts`.
...
---
_Generated: 2026-08-04 (resynced — added `components/shared/data-table/data-table-buttons.tsx` and `components/ui/button-group.tsx` rows; corrected `data-table.tsx`/`data-table-toolbar.tsx` import lists and behavior after the toolbar restructure and button-cluster extraction; corrected `notifications-data-table.tsx`'s filter-apply behavior) — scope: client app "admin" — see .claude/CLAUDE.md for update rules._
