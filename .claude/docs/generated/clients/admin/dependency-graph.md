...
| `features/notifications/components/notifications-data-table.tsx` | `components/shared/data-table` (barrel), `components/ui/native-select.tsx`, `features/notifications/components/{send-notification-dialog,user-select}.tsx`, `lib/shared/user-display.ts`, `features/notifications/types/notification.ts`, `types/user.ts` | Status + recipient filter dropdowns wired to URL params. Status filter switched from `components/ui/select.tsx` (Radix, removed this sync) to `native-select.tsx`, with a real reselectable placeholder replacing a fake "all" option |
...
| `components/shared/data-table/data-table.tsx` | `components/ui/{table,skeleton,empty,alert}.tsx`, `./data-table-toolbar.tsx`, `./data-table-pagination.tsx`, `./data-table-virtual-body.tsx` (new), `./types.ts`, `lucide-react` (`ArrowUp`/`ArrowDown`/`ArrowUpDown`) | Exported via `./index.ts` barrel. Optional per-column client-side sort (`sortState`/`toggleSort`/a `sortedData` `useMemo`) from a prior sync. **New this sync**: `mode` prop (`"paginated"`/`"virtualized"`/`"infinite"`) and `onSortChange` for server-driven sort, both additive |
| `components/shared/data-table/data-table-virtual-body.tsx` | `lib/shared/utils.ts`, `components/foundation/use-virtual-list.ts`, `./types.ts` | New — row renderer for `mode="virtualized"`/`"infinite"`; ARIA-grid div layout, not a native `<table>` |
...
---
_Generated: 2026-08-04 (resynced — removed `components/select/*` and its dependent `components/foundation/*` rows; added `components/ui/{popover,command,combobox}.tsx`, `components/foundation/portal-container.ts`, and `cmdk` rows; updated `edit-user-dialog`/`user-select`/`use-sidebar` rows; corrected the "`components/ui/*` is a strict leaf layer" claim in Circular References) — scope: client app "admin" — see .claude/CLAUDE.md for update rules._
