...
| `Identity.Contracts`'s new `IdentityPermissionProvider` uses vendor `Light.AspNetCore.Authorization` types (`IPermissionDefinitionProvider`, `PermissionDefinition`) without `Identity.Contracts.csproj` declaring `Lightsoft.AspNetCore.Authorization` as a direct `PackageReference` — it rides in transitively via the new `ProjectReference` to `Shared` | Low | Same undeclared-transitive-dependency pattern already flagged for `Identity.Api`'s use of `Light.EntityFrameworkCore.Extensions.WhereIf` (see `coding-conventions.md` § Deviations, `dependency-graph.md`). |
...
---
_Generated: 2026-08-03 (resynced — `PermissionsController`/`IdentityPermissionProvider` permission catalog added; `Identity.Contracts` no longer a leaf) — scope: Backend — see .claude/CLAUDE.md for update rules._
