...
| Vendor `Lightsoft.SmtpMail` (`Notifications.Api`) | package, **direct**, preview (`1.1.1-preview3`) — **new this sync** | `ISmtpMailSender`/`SmtpMailKitOptions` (`Light.Mail`/`Light.SmtpMail` namespaces) backing `MailService`. This is `Notifications.Api`'s **first ever direct `<PackageReference>`** — every other vendor type it uses rides in transitively via `Infrastructure`/`Persistence`/`Shared` (see `docs/generated/backend/dependency-graph.md`). |
...
---
_Generated: 2026-08-03 — scope: module "Notifications" (first-time generation) — see .claude/CLAUDE.md for update rules. Resynced 2026-08-11 — added the new `IMailService`/`MailService` mail-sending capability (Contracts + Api layering, Public Contract, Dependencies, Notable Conventions) and the new `Identity.Api → Notifications.Contracts` cross-module dependency it enables (Depended On By)._
