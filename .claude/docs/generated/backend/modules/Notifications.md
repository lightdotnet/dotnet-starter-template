...
**Mail (new this sync, not HTTP-exposed)**: `IMailService` (`Notifications.Contracts/Services/IMailService.cs`) is not surfaced by any controller — it's consumed in-process by other modules via C# DI, not over HTTP. `MailService` (`Notifications.Api/Services/MailService.cs`, `internal`) implements it on top of vendor `ISmtpMailSender`/`SmtpMailKitOptions` (`Light.Mail`/`Light.SmtpMail`): `SendFromSystemAsync` sends from `options.UserName` (labeled `"System"` — a placeholder "system" sender address, per an inline comment "fake system email address for now, we can change it later"); `SendAsync` sends from an explicit `from` address. First real consumer: `Identity.Api`'s `UserCreatedEventHandler` (see `docs/generated/backend/modules/Identity.md`).
...
| Vendor `Lightsoft.SmtpMail` (`Notifications.Api`) | package, **direct** (`2.0.0`) | `ISmtpMailSender`/`SmtpMailKitOptions` (`Light.Mail`/`Light.SmtpMail` namespaces) backing `MailService`. This is `Notifications.Api`'s **first ever direct `<PackageReference>`** — every other vendor type it uses rides in transitively via `Infrastructure`/`Persistence`/`Shared` (see `docs/generated/backend/dependency-graph.md`). |
...
**Changed this sync**: `Identity.Api` now references `Notifications.Contracts` — its `UserCreatedEventHandler` takes a DI-injected `IMailService` and calls `SendFromSystemAsync(...)` to send a welcome email on user creation (wrapped in try/catch; a failure only logs, it doesn't fail the request). This is the first cross-module reference between `Identity` and `Notifications` in either direction, and it's compliant with the module-boundary rule — it reaches only `Notifications.Contracts` (the seam), never `Notifications.Api`'s internals. `Notifications.Api` itself is still referenced only by `StarterKit.WebApi` (composition-root host) and `src/Migrations/MSSQL/MSSQL.csproj` (migrations tooling) — verified via `ProjectReference` search across all `.csproj` files under `src/`. Client-side: `clients/admin/src/features/notifications/` is the only verified consumer of the module's HTTP surface, calling `notification`/`user_notification` over HTTP and connecting directly to `/signalr-hub` (proxied same-origin via the admin app's `next.config.ts` `rewrites()`); the admin client has no path to `IMailService` — it's an in-process C# service, not an HTTP endpoint.
...
- **New this sync**: `MailService.SendFromSystemAsync` sends from `options.UserName` labeled as `"System"` — per its own inline comment, this is a placeholder ("fake system email address for now, we can change it later"), not a dedicated no-reply address; don't assume it's production-ready as-is.
- **New this sync**: `NotificationModule.AddSmtpMail` reads the `SmtpMail` configuration section (`Host`/`Port`/`UseSsl`/`UserName`/`Password`) via `configuration.GetSection("SmtpMail").Get<SmtpMailKitOptions>()` and throws `InvalidOperationException` at startup if the section is missing entirely (not if individual values are empty/blank — `src/StarterKit.WebApi/appsettings.json`'s `UserName`/`Password` are currently empty strings, which satisfies this check but would fail at actual send time against a real SMTP server).
...
---
_Generated: 2026-08-03 — scope: module "Notifications" (first-time generation) — see .claude/CLAUDE.md for update rules. Resynced 2026-08-11 — added the new `IMailService`/`MailService` mail-sending capability (Contracts + Api layering, Public Contract, Dependencies, Notable Conventions) and the new `Identity.Api → Notifications.Contracts` cross-module dependency it enables (Depended On By). Resynced 2026-08-18 — `Lightsoft.SmtpMail` moved off preview to stable `2.0.0`._
