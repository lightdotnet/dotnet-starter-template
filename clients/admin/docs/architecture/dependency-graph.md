...
- **Server-only guard**: `server-only` — compile-time marker forcing a build error if a `lib/server/*` module is ever imported into a Client Component; present in all 17 files under `lib/server/` (the 16 flat files plus `http-handlers/bearer-token-handler.ts`).
